...
def build_automaton(ct: Countertrace, cp: str = "c") -> PhaseEventAutomaton:
    ...
    while pending or init:
        if init:
            ...
            visited.add(p)
            src = Phase(
                compute_state_invariant(ct, p),
                compute_clock_invariant(ct, p, cp),
                p,
            )
        ...
        for s in successors:
            dst = Phase(
                compute_state_invariant(ct, s[0]),
                compute_clock_invariant(ct, s[0], cp),
                s[0],
            )
...
def compute_clock_invariant(ct: Countertrace, p: Sets, cp: str) -> FNode:
    ...
    for i in p.active:
        ...
        if not (i in p.wait) and (
            ct.dc_phases[i].is_upper_bound() and can_seep(p, i) == FALSE()
        ):
            result.append(LE(*lt_args))
...
def build_successors(
    ...
    keep: dict[int, FNode],
    cp: str,
) -> list[tuple[Sets, FNode, set[str]]]:
    result = []
    guard = guard.simplify()

    # Terminate if guard is unsatisfiable.
    if guard != TRUE() and (
        guard == FALSE() or not is_sat(guard, solver_name=SOLVER_NAME, logic=LOGIC)
    ):
        return []
    ...
    if ct.dc_phases[i].is_lower_bound():
            ...
            result.extend(
                build_successors(
                    ...
                    And(
                        guard,
                        keep[i],
                        GE(Symbol(cp + str(i), REAL), ct.dc_phases[i].bound),
                    ),
                    ct,
                    enter,
                    keep,
        ...
        result.extend(
            build_successors(
                i + 1, p, p_.add_active(i), resets, guard, ct, enter, keep, cp
            )
        )
...
def compute_enter_keep(
    ct: Countertrace, p: Sets, init: bool, cp: str
