...
def replace_var_in_expression(expression, replacement_mapping, parser=None, matching_terminal_names=('ID')):
    """ Replaces all occurrences of old_var in expression with new_var.

    :param replacement_mapping: {'old_name': 'new_name'}
    :type replacement_mapping: dict
    ...
    for node in tree.iter_subtrees():  # type: Tree
        for child in node.children:
            ...
            if isinstance(child, Token) and child.type in matching_terminal_names:
                if child.value in replacement_mapping.keys():
                    node.set(data=node.data, children=[Token(child.type, replacement_mapping[child.value])])
