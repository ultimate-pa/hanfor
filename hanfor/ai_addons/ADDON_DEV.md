...
class MyAddon(AiAddonAbstractClass):
    ...
    def __init__(self, enabled: bool, thread_handler, ai_request, socketio):
...
Use the decorator below for methods that should **do nothing when the
addon is disabled**:
...
If you want to send updates **only to one client**, you must track the
`sid`.
