...
<!-- Other boxes: tip, note, caution, danger, arbitrary text --> 
!!! note "Example"
...
!!! note "**Input**"
    Requirements to be tested:<br>
    ``req1: Globally, it is always the case that if ‘A’ holds then ‘H’ holds after at most ‘10’ time units.``<br>
    ``req2: Globally, it is always the case that if ‘B’ holds then ‘I’ holds after at most ‘10’ time units.``<br>
    ``req3: Globally, it is always the case that if ‘H AND I’ holds then ‘O’ holds after at most ‘10’ time units.``<br>
...
!!! note "**Output** of the Test Generator Tool"
    ````
    Case SystemTest:
    TestGeneratorResult:
    Found Test for: [O]
