...
results = [[{'if_part': ['if_case'], 'logic': 'AND-logic', 'then_part': ['then_case']}],  # 1

           [{'if_part': ['if_case'], 'logic': 'AND-logic', 'then_part': ['then_case'], 'else_part': ['else_case']}],
           # 2

           [{'if_part':   ['if_case'], 'logic': 'AND-logic',
           ...
           [{'then_part': ['so_RLS_01__RS_Verbau_KCAN := so_RLS_01__RS_Verbau_KCAN_REGENSENSOR NICHT VERBAUT ',
           ...
           [{'then_part': ['so_RLS_01__SLS_Helligkeit_IR_KCAN := 7000', 'so_RLS_01__SLS_Helligkeit_IR_SCAN := 7000'],
             'if_part':   ['s_ls_in_lux_messwert > 7000'], 'logic': 'AND-logic'
             }],  # 12

           [{'logic':     'AND-logic',
             'then_part': ['s_ls_anaout := s_helligkeit_uin_mM',
                           's_ls_in := ( ( adp_s_outmax - s_ls_anaout ) * ( adp_s_inpmax - adp_s_inpmin ) / ( '
                           'adp_s_outmax - adp_s_outmin ) )',
                           's_ls_in_lux_messwert := s_ls_in'],
             'if_part':   ['adp_Lichtsensor_Typ == adp_Lichtsensor_Typ_SMART_LS ',
                           's_helligkeit_uin_mM <= adp_s_outmax']
             }],  # 13
